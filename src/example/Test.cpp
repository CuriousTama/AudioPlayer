...
void test_SubMixer(Audio_System& _audioSystem, std::string path)
{
    _audioSystem.addSubMixer("Custom1");
    Sound sound(path);
    sound.setMixer("CUSTOM1");
    sound.play();

    while (_audioSystem.getSubMixerVolume("custom1") > 1.f)
    {
        std::this_thread::sleep_for(std::chrono::milliseconds(50));
        _audioSystem.setSubMixerVolume("custom1", _audioSystem.getSubMixerVolume("custom1") - 1.f);
    }
}
void test_SubMixer3D(Audio_System& _audioSystem, std::string path)
{
    _audioSystem.setSubMixerVolume("Custom1", 100.f);
    Sound sound(path);
...
{
    Sound sound(path);
    sound.setReverb(ReverbParameters::Concerthall);
    sound.play();
...
void tests(std::string SoundPath)
{
    ...
    audioSystem.setMasterVolume(100.f);
    ...
    test_SubMixer(audioSystem, SoundPath);

    std::cout << "Test : Submixer3D\n";
    test_SubMixer3D(audioSystem, SoundPath);
